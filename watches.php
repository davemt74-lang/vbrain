...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    try{
        ...
        if($action==='add_trip'){$tripId=(int)($_POST['trip_id']??0);$service->saveTrip($userId,$tripId,['watch_weather'=>1,'watch_flights'=>1,'watch_events'=>1,'watch_places'=>1,'interval_minutes'=>$_POST['interval_minutes']??360,'is_active'=>1]);flash('success','Trip watch activated. Vacation Brain will watch for meaningful changes.');}
...
function watch_time_label(mixed $value): string{
    $value=trim((string)$value);if($value==='')return 'Not yet';$ts=strtotime($value);if(!$ts)return 'Not yet';$delta=time()-$ts;if($delta>=0){if($delta<60)return 'Just now';if($delta<3600)return (int)floor($delta/60).'m ago';if($delta<86400)return (int)floor($delta/3600).'h ago';return (int)floor($delta/86400).'d ago';}$future=abs($delta);if($future<3600)return 'in '.max(1,(int)ceil($future/60)).'m';if($future<86400)return 'in '.max(1,(int)ceil($future/3600)).'h';return 'in '.max(1,(int)ceil($future/86400)).'d';
}
function watch_signal_label(string $type): string{return ['weather'=>'Weather','flights'=>'Flights','events'=>'Events','places'=>'Local'][$type]??ucfirst($type);}
function watch_event_url(array $row): string{
    $meta=$row['metadata']??[];$tab=(string)($meta['target_tab']??'overview');if((int)($row['dream_trip_id']??0)>0)return app_url('dream-trip.php?id='.(int)$row['dream_trip_id'].'&tab='.rawurlencode($tab));if((int)($row['destination_catalog_id']??0)>0)return app_url('destination-report.php?destination_id='.(int)$row['destination_catalog_id']);return app_url('watches.php');
}
require __DIR__.'/partials/header.php';
?>
<section class="dashboard travel-watches-page"><div class="shell">
  <div class="travel-watch-head"><div><span class="eyebrow">Vacation Brain monitors</span><h1>Destination Watches</h1><p>Watch the parts of a trip that actually change: weather, airfare, events, and local options.</p></div><div class="travel-watch-head-actions"><a class="button secondary small" href="<?=e(app_url('dream.php'))?>">Plan a trip</a><a class="button secondary small" href="<?=e(app_url('notifications.php'))?>">Alerts</a></div></div>
  <?php if($success):?><div class="alert success"><?=e($success)?></div><?php endif;?>
  <?php if($error):?><div class="alert error"><?=e($error)?></div><?php endif;?>
  <?php if(!$ready):?><div class="trip-intel-alert"><strong>Destination Watches need the latest database upgrade.</strong> <?php if(is_admin()):?><a href="<?=e(app_url('upgrade.php'))?>">Run System Upgrade</a><?php else:?>An administrator needs to apply migration 036.<?php endif;?></div><?php endif;?>

  <section class="dashboard-card travel-watch-create">
    <div><span class="eyebrow">Watch a planned trip</span><h2>Give the agents something to keep an eye on.</h2><p>Trip watches can monitor all four specialist data streams, including route-specific airfare when flight provider access is configured.</p></div>
  ...
  <div class="travel-watch-layout">
    ...
    <aside class="travel-watch-stack">
      ...
      <section class="travel-watch-how"><span class="eyebrow">Noise control</span><h2>Useful changes, not every twitch.</h2><p>Watches use stricter thresholds than the live trip workspace: meaningful fare movement, substantial forecast changes, new events, and notable local-result changes.</p><?php if(is_admin()):?><code>php bin/run-travel-watches.php --limit=50</code><small>Schedule this command hourly on the production server. Per-watch cadence controls whether each watch is actually due.</small><?php endif;?></section>
    </aside>
  </div>
</div></section>
