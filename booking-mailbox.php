...
$success=flash('success');$oauthError=flash('error');if($oauthError)$error=$oauthError;
$connection=$service->ready()?$service->connection($userId):null;$changes=$service->ready()?$service->changes($userId,'needs_review',60):[];$messages=$service->ready()?$service->messages($userId,60):[];$configured=$service->googleConfigured();
    ...
    <div class="dashboard-card vb-mail-card">
      ...
      <?php else:?>
        ...
        <div class="vb-mail-actions">
          <?php if($connection['status']==='error'):?><form method="post"><input type="hidden" name="_csrf" value="<?=e(csrf_token())?>"><input type="hidden" name="action" value="connect"><button class="button primary small" type="submit">Reconnect Gmail</button></form><?php else:?><form method="post"><input type="hidden" name="_csrf" value="<?=e(csrf_token())?>"><input type="hidden" name="action" value="sync"><button class="button primary small" type="submit" <?=$connection['status']==='paused'?'disabled':''?>>Sync now</button></form><?php endif;?>
          <?php if($connection['status']==='paused'):?><form method="post"><input type="hidden" name="_csrf" value="<?=e(csrf_token())?>"><input type="hidden" name="action" value="resume"><button class="button secondary small" type="submit">Resume</button></form><?php elseif($connection['status']==='connected'):?><form method="post"><input type="hidden" name="_csrf" value="<?=e(csrf_token())?>"><input type="hidden" name="action" value="pause"><button class="button secondary small" type="submit">Pause</button></form><?php endif;?>
        </div>
        <form method="post" class="vb-mail-form"><input type="hidden" name="_csrf" value="<?=e(csrf_token())?>"><input type="hidden" name="action" value="save_settings">
          ...
          <label class="vb-check"><input type="checkbox" name="use_ai" value="1" <?=$connection['use_ai']?'checked':''?>> Use the configured AI model to improve connected-mail extraction <small>Off by default. Sender identity, confirmation codes, and payment-card/security-code data are removed before this text is sent to your configured model.</small></label>
