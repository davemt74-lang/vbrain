...
final class TripAgentService
{
    ...
    public function send(int $userId,int $tripId,string $agentType,string $message,?string $idempotencyKey=null,?array $dashboardOverride=null,?int $batchId=null): array
    {
        ...
        if($idempotencyKey!==null&&trim($idempotencyKey)!==''){
            ...
            if($existing){$meta=json_decode((string)($existing['metadata_json']??''),true)?:[];return ['agent_type'=>$agentType,'message'=>(string)$existing['body'],'data_health'=>$meta['data_health']??[],'intelligence_batch_id'=>$meta['intelligence_batch_id']??null,'reused'=>true];}
        }
        $dashboard=$dashboardOverride??(new TripIntelligenceService($this->pdo))->dashboard($userId,$tripId);$dashboardTrip=$dashboard['trip']??[];if((int)($dashboardTrip['id']??0)!==$tripId||(int)($dashboardTrip['user_id']??0)!==$userId)throw new RuntimeException('Trip-agent context does not match this trip.');
        $batch=is_array($dashboard['_agent_batch']??null)?$dashboard['_agent_batch']:[];$batchId=$batchId??(!empty($batch['id'])?(int)$batch['id']:null);$snapshotIds=is_array($batch['snapshot_ids']??null)?$batch['snapshot_ids']:[];
        $this->insert($userId,$tripId,$agentType,'user',$message,$userFingerprint,['kind'=>'user','job_key'=>$idempotencyKey,'intelligence_batch_id'=>$batchId]);$system=$this->systemPrompt($agentType,$dashboard);$context=$this->datasetContext($agentType,$dashboard);$recent=$this->history($userId,$tripId,$agentType,12);$transcript=[];foreach($recent as $row){if(($row['metadata']['kind']??'')==='proactive')continue;$transcript[]=strtoupper((string)$row['role']).': '.(string)$row['body'];}$input="TRIP DATA\n".$context."\n\nRECENT ".strtoupper($agentType)." AGENT CONVERSATION\n".implode("\n",array_slice($transcript,-10))."\n\nUSER REQUEST\n".$message;
        ...
        $health=$this->live->scopedHealth($agentType,$dashboard);$this->insert($userId,$tripId,$agentType,'assistant',$reply,$assistantFingerprint,['kind'=>'agent_result','data_scope'=>$this->scope($agentType),'data_health'=>$health,'job_key'=>$idempotencyKey,'intelligence_batch_id'=>$batchId,'snapshot_ids'=>$snapshotIds]);return ['agent_type'=>$agentType,'message'=>$reply,'data_health'=>$health,'intelligence_batch_id'=>$batchId,'reused'=>false];
    }
    ...
    {
        $label=$this->label($agentType);$scope=implode(', ',$this->scope($agentType));$batch=is_array($dashboard['_agent_batch']??null)?$dashboard['_agent_batch']:[];$batchRule=$batch?' This request is attached to one immutable shared intelligence batch. Analyze only that batch for provider facts so all trip agents reason from the same snapshot; do not replace it with newer provider data during this run.':'';
        return "You are Vacation Brain's $label for one specific planned trip. Your allowed data scope is: $scope. Be concise, practical and proactive. Use the supplied trip/provider data as facts. Never invent live weather, fares, events, ratings, availability, prices or booking inventory. For every time-sensitive recommendation, respect the supplied provider, freshness state, expiry state and latest-change signal. If data is stale, historical, indicative, limited, unavailable or unconfigured, say that plainly before giving advice. Skyscanner indicative fares are cached market intelligence and are not guaranteed bookable prices. Lodging is a paid-partnership surface: do not fabricate hotel inventory. Suggest concrete next actions and scheduling improvements when supported by the data. The Overview Agent may reason across all supplied datasets and prioritize booking/planning actions. Specialist agents should stay focused on their named dataset except for weather-fit context explicitly supplied to Events/Local/Itinerary.".$batchRule;
    }

    private function datasetContext(string $agentType,array $dashboard): string
    {
        $trip=$dashboard['trip']??[];$base=['trip'=>['id'=>$trip['id']??null,'name'=>$trip['name']??'','destination'=>$trip['destination_name']??'','origin'=>$trip['origin_name']??'','origin_iata'=>$trip['origin_iata']??'','destination_iata'=>$trip['destination_iata']??'','start_date'=>$trip['start_date']??null,'end_date'=>$trip['end_date']??null,'travelers'=>$trip['travelers']??1,'target_budget'=>$trip['target_budget']??null,'status'=>$trip['status']??'','booking_readiness'=>$trip['booking_readiness']??0]];$snap=$dashboard['snapshots']??[];$data=$base;$userId=(int)($trip['user_id']??0);$tripId=(int)($trip['id']??0);$batch=is_array($dashboard['_agent_batch']??null)?$dashboard['_agent_batch']:[];
        $data['live_status']=$this->live->scopedHealth($agentType,$dashboard);$changes=is_array($batch['changes']??null)?$batch['changes']:($userId>0&&$tripId>0?$this->live->allChanges($userId,$tripId):[]);$data['latest_changes']=$this->scopedChanges($agentType,$changes);
        if($batch)$data['analysis_batch']=['id'=>$batch['id']??null,'prepared_at'=>$batch['prepared_at']??null,'scope'=>$batch['scope']??null,'refreshed_types'=>$batch['refreshed_types']??[],'snapshot_ids'=>$batch['snapshot_ids']??[]];
        if($agentType==='overview'){$super=is_array($batch['supervisor']??null)?$batch['supervisor']:(new TripSupervisorService($this->pdo))->overview((int)$trip['user_id'],(int)$trip['id'],$dashboard);$data+=['weather'=>$this->payload($snap,'weather'),'flights'=>$this->payload($snap,'flights'),'events'=>$this->payload($snap,'events'),'places'=>$this->payload($snap,'places'),'opportunities'=>$dashboard['opportunities']??[],'budget'=>$dashboard['budget']??[],'itinerary'=>$this->compactItems($trip['items']??[]),'updates'=>$super['updates']??[],'proactive_suggestions'=>$super['suggestions']??[],'agent_states'=>$super['agent_states']??[],'specialist_results'=>$batch['specialist_results']??[]];}
        ...
        $json=json_encode($data,JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE|JSON_INVALID_UTF8_SUBSTITUTE);return $json!==false?$this->clip($json,36000):'{}';
    }

    private function fallback(string $agentType,array $dashboard): string
    {
        $batch=is_array($dashboard['_agent_batch']??null)?$dashboard['_agent_batch']:[];$super=is_array($batch['supervisor']??null)?$batch['supervisor']:[];$result=[];
        if($super){if($agentType==='overview'){$top=$super['suggestions'][0]??null;$result=['body'=>$top?((string)($top['title']??'Trip update').' — '.(string)($top['body']??'')):'Shared trip intelligence is ready for review.','next_action'=>$top?(string)($top['body']??''):'Review the current specialist results.','source'=>'Vacation Brain','freshness'=>(string)($batch['prepared_at']??'')];}else{$candidate=$super['agent_states'][$agentType]??null;if(is_array($candidate))$result=$candidate;}}
        if(!$result)$result=(new TripSupervisorService($this->pdo))->activeResult($agentType,$dashboard);$body=trim((string)($result['body']??''));$next=trim((string)($result['next_action']??''));$source=trim((string)($result['source']??''));$fresh=trim((string)($result['freshness']??''));$parts=[];if($body!=='')$parts[]=$body;if($source!==''&&$source!=='Vacation Brain')$parts[]='Source: '.$source.($fresh!==''?' · '.$fresh:'').'.';if($next!=='')$parts[]='Next move: '.$next;$parts[]='The configured LLM is unavailable right now, but the trip intelligence and planning calculations above are still active.';return implode(' ',$parts);
    }
