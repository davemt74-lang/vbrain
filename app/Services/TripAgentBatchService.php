...
final class TripAgentBatchService
{
    ...
    private function prepareClaimed(array $batch,string $token): void
    {
        ...
        $dashboard['_agent_batch']=['id'=>$id,'scope'=>(string)$batch['scope'],'agent_types'=>$agents,'required_types'=>$required,'refreshed_types'=>$refresh,'snapshot_ids'=>$snapshotIds,'prepared_at'=>gmdate('c'),'changes'=>$live->allChanges($userId,$tripId),'updates'=>$supervisor['updates']??[],'data_health'=>$supervisor['data_health']??[]];
