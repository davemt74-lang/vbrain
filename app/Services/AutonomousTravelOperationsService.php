...
final class AutonomousTravelOperationsService
{
    ...
    public function saveSettings(int $userId,int $tripId,array $input): array
    {
        ...
        $minimum=max(0,min(100,(int)($input['minimum_priority']??90)));$starts=max(0,min(24,(int)($input['max_auto_starts_per_day']??3)));$applies=max(0,min(24,(int)($input['max_auto_applies_per_day']??2)));$perAction=$this->money($input['per_action_estimated_limit']??0);$daily=$this->money($input['daily_estimated_limit']??0);
        ...
        $this->pdo->prepare($sql)->execute([$userId,$tripId,!empty($input['enabled'])?1:0,$mode,$minimum,$starts,$applies,$perAction,$daily,$this->json($types),!empty($input['pause_on_verification_pending'])?1:0,!empty($input['notifications_enabled'])?1:0]);return $this->settings($userId,$tripId);
    }
