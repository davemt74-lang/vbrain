...
final class TripTravelOperationsService
{
    ...
    public function snapshot(int $userId,int $tripId,?string $date=null,bool $sync=true): array
    {
        ...
        if(!$this->ready())return [
            'ready'=>false,'generated_at'=>date(DATE_ATOM),'trip'=>$this->publicTrip($trip,'planning'),'state'=>'planning','state_label'=>'Planning','selected_date'=>$selected,
            'timeline'=>[],'timeline_groups'=>['now'=>[],'next'=>[],'later'=>[],'earlier'=>[]],'wallet'=>[],'flights'=>[],'lodging'=>[],'alerts'=>[],'readiness'=>[],
            'countdown'=>$this->countdown($trip,[],'planning'),'agents'=>[],'watch_signals'=>[],'navigation'=>$this->navigation($trip,$selected),'offline'=>[]
        ];
        ...
        $state=$this->deriveState($trip,$readiness);if($sync)$state=$this->syncState($userId,$tripId,$trip,$state,'automatic');
        $timeline=$this->buildTimeline($trip,$bookings,$selected);$alerts=$this->alerts($userId,$tripId,$bookings,$requirements);
        ...
        $snapshot=[
            ...
            'countdown'=>$this->countdown($trip,$readiness,$state),'readiness'=>$readiness,'timeline'=>$timeline,'timeline_groups'=>$this->timelineGroups($timeline),
        ...
        $snapshot['offline']=$this->offlineSnapshot($snapshot);return $snapshot;
    }
    ...
    public function setState(int $userId,int $tripId,string $state,string $reason='manual'): string
    {
        ...
        $trip=(new DreamService($this->pdo))->get($userId,$tripId,false);if(!$trip)throw new OutOfBoundsException('Trip not found.');return $this->syncState($userId,$tripId,$trip,$state,$reason);
    }
    ...
    public function updateBookingOperations(int $userId,int $tripId,int $bookingId,array $input): array
    {
        ...
        if($checkin!==null&&!preg_match('#^https?://#i',$checkin))throw new InvalidArgumentException('Check-in URL must begin with http:// or https://.');$changed=$status!==(string)($booking['operational_status']??'unknown');
        $stmt=$this->pdo->prepare("UPDATE trip_bookings SET terminal=?,gate=?,location_name=?,location_address=?,operational_status=?,operational_note=?,status_source='manual',last_status_at=IF(?<>'unknown',NOW(),last_status_at),checkin_url=?,updated_at=NOW() WHERE id=? AND user_id=? AND dream_trip_id=?");
        $stmt->execute([$terminal,$gate,$location,$address,$status,$note,$status,$checkin,$bookingId,$userId,$tripId]);$updated=$this->ownedBooking($userId,$tripId,$bookingId);if(!$updated)throw new RuntimeException('Booking operation update failed.');
        if($changed)$this->recordEvent($userId,$tripId,$bookingId,'booking_status','low','Travel status updated',(string)$updated['title'].' is now '.str_replace('_',' ',$status).'.','booking-status:'.$bookingId.':'.$status,['status'=>$status,'source'=>'manual']);return $updated;
    }
    ...
    public function augmentCommandCenterSnapshot(int $userId,array $snapshot): array
    {
        ...
        foreach($trips as &$trip){$tripId=(int)($trip['id']??0);if($tripId<1)continue;try{$state=$this->tripState($userId,$tripId);$days=$trip['days_until']??null;$withinWeek=is_numeric($days)&&(int)$days>=0&&(int)$days<=7;$trip['operational_state']=$state;$trip['operational_state_label']=$this->stateLabel($state);$trip['travel_mode_url']=app_url('travel-mode.php?id='.$tripId);$trip['travel_mode_prominent']=in_array($state,['ready','traveling'],true)||$withinWeek;}catch(Throwable $e){}}unset($trip);$snapshot['upcoming_trips']=$trips;return $snapshot;
    }
    ...
    private function deriveState(array $trip,array $readiness): string
    {
        ...
        if($current==='completed'||($trip['status']??'')==='completed'||($end!==''&&$end<$today))return 'completed';if($current==='traveling')return 'traveling';if($start!==''&&$start<=$today&&($end===''||$end>=$today))return 'traveling';if($current==='ready')return 'ready';
        $days=$start!==''?(int)floor((strtotime($start)-strtotime($today))/86400):null;$pct=(int)($readiness['percent']??0);if($days!==null&&$days>=0&&$days<=7&&$pct>=90)return 'ready';if((int)($readiness['bookings_total']??0)>0||($trip['status']??'')==='booked')return 'booking';return 'planning';
    }
    ...
    {
        $items=[];$now=time();$today=date('Y-m-d');foreach($bookings as $b){$status=(string)($b['status']??'');if(!in_array($status,['booked','confirmed','changed'],true))continue;$starts=(string)($b['starts_at']??'');$ends=(string)($b['ends_at']??'');$type=(string)($b['booking_type']??'other');$include=$starts!==''&&substr($starts,0,10)===$selected;if($type==='lodging'&&$starts!==''&&$ends!==''&&substr($starts,0,10)<=$selected&&substr($ends,0,10)>=$selected)$include=true;if(!$include)continue;
    ...
    private function timelineGroups(array $timeline): array{$groups=['now'=>[],'next'=>[],'later'=>[],'earlier'=>[]];foreach($timeline as $item){$phase=(string)($item['phase']??'later');if(!isset($groups[$phase]))$phase='later';$groups[$phase][]=$item;}return $groups;}

    private function alerts(int $userId,int $tripId,array $bookings,array $requirements): array
    {
        $out=[];$now=time();$day=$now+86400;$three=$now+3*86400;foreach($bookings as $b){$bookingId=(int)$b['id'];$status=(string)$b['status'];$ops=(string)($b['operational_status']??'unknown');$type=(string)$b['booking_type'];
            if($status==='cancelled'||$ops==='cancelled')$out[]=$this->alert('booking-cancelled:'.$bookingId,'high','Reservation cancelled',(string)$b['title'].' needs a recovery plan.',$bookingId,$type,98);elseif($status==='changed')$out[]=$this->alert('booking-changed:'.$bookingId,'high','Reservation changed',(string)$b['title'].' changed. Reconfirm the live details and adjust the trip if needed.',$bookingId,$type,95);elseif($ops==='delayed')$out[]=$this->alert('booking-delayed:'.$bookingId,'high','Travel delay recorded',(string)$b['title'].' is marked delayed. Review downstream timing and alternatives.',$bookingId,$type,94);
            $check=$this->ts($b['checkin_opens_at']??null);if($check&&$check>=$now&&$check<=$day&&in_array($status,['booked','confirmed'],true))$out[]=$this->alert('checkin:'.$bookingId,'medium','Check-in is opening soon',(string)$b['title'].' can be checked in within 24 hours.',$bookingId,$type,80);$cancel=$this->ts($b['cancellation_deadline']??null);if($cancel&&$cancel>=$now&&$cancel<=$day&&$status!=='cancelled')$out[]=$this->alert('cancel-deadline:'.$bookingId,'medium','Cancellation deadline is close',(string)$b['title'].' reaches its cancellation deadline within 24 hours.',$bookingId,$type,82);}
    ...
    {
        if(!db_table_exists('trip_agent_actions'))return;$active=[];foreach($alerts as $alert){if(($alert['source']??'')!=='booking'||(int)($alert['priority']??0)<90)continue;$key=(string)$alert['key'];$sourceKey='travel-ops:'.substr(hash('sha256',$key.'|'.$alert['title'].'|'.$alert['body']),0,48);$active[]=$sourceKey;$agent=(string)($alert['target_agent']??'itinerary');$meta=['source'=>'travel_day_operations','signal_key'=>$key,'booking_id'=>$alert['booking_id']??null,'requires_user_approval'=>true,'external_booking_not_completed'=>true];$json=json_encode($meta,JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE|JSON_INVALID_UTF8_SUBSTITUTE)?:'{}';
            $sql="INSERT INTO trip_agent_actions (user_id,dream_trip_id,batch_id,suggestion_key,source_key,agent_type,action_kind,title,body,priority,target_tab,status,metadata_json) VALUES (?,?,NULL,?,?,?,?,?,?,?,?,'open',?) ON DUPLICATE KEY UPDATE title=VALUES(title),body=VALUES(body),priority=VALUES(priority),metadata_json=VALUES(metadata_json),status=IF(status IN ('accepted','dismissed','completed'),status,'open'),updated_at=NOW()";$this->pdo->prepare($sql)->execute([$userId,$tripId,substr('recovery-'.$key,0,80),$sourceKey,$agent,'recovery',(string)$alert['title'],(string)$alert['body'].' Vacation Brain can propose a recovery plan, but any replacement booking still requires live provider confirmation.',(int)$alert['priority'],$agent,$json]);
        ...
        if($active){$marks=implode(',',array_fill(0,count($active),'?'));$params=array_merge([$userId,$tripId],$active);$stmt=$this->pdo->prepare("UPDATE trip_agent_actions SET status='superseded',updated_at=NOW() WHERE user_id=? AND dream_trip_id=? AND status='open' AND source_key LIKE 'travel-ops:%' AND source_key NOT IN ($marks)");$stmt->execute($params);}else{$this->pdo->prepare("UPDATE trip_agent_actions SET status='superseded',updated_at=NOW() WHERE user_id=? AND dream_trip_id=? AND status='open' AND source_key LIKE 'travel-ops:%'")->execute([$userId,$tripId]);}
    }
    ...
    private function selectedDate(?string $date,array $trip): string{$selected=$this->dateOrToday($date);$start=trim((string)($trip['start_date']??''));$end=trim((string)($trip['end_date']??''));if($start!==''&&$selected<$start)$selected=$start;if($end!==''&&$selected>$end)$selected=$end;return $selected;}
