...
final class TripAgentExecutionService
{
    ...
    public function acceptAndStart(int $userId,int $tripId,int $actionId): array
    {
        ...
        $agent=$this->agent((string)($action['agent_type']??$action['target_tab']??'overview'));
    ...
    public function dispatchQueued(int $limit=10): array
    {
        ...
        foreach($rows as $id){$result['checked']++;try{$out=$this->dispatchExecution((int)$id);if(($out['status']??'')==='agent_working'){$result['dispatched']++;$result['executions'][]=(int)$id;}else$result['deferred']++;}catch(Throwable $e){$result['failed']++;$this->markFailed((int)$id,$e->getMessage());}}
        return $result;
    }
    ...
    public function syncFinishedJobs(int $limit=25): array
    {
        ...
        foreach($rows as $row){$result['checked']++;$id=(int)$row['id'];if((string)$row['job_status']!=='completed'){$this->markFailed($id,(string)($row['job_error']?:'The specialist agent did not complete.'));$result['failed']++;continue;}
            try{$this->createProposal($id,(string)($row['result_json']??''));$result['proposed']++;$result['executions'][]=$id;}catch(Throwable $e){$this->markFailed($id,$e->getMessage());$result['failed']++;}}
        return $result;
    }

    public function executionsForActions(int $userId,int $tripId,array $actionIds): array
    {
        if(!$this->ready()||!$actionIds)return [];$ids=array_values(array_unique(array_filter(array_map('intval',$actionIds),fn($v)=>$v>0)));if(!$ids)return [];$marks=implode(',',array_fill(0,count($ids),'?'));$params=array_merge([$userId,$tripId],$ids);
        $stmt=$this->pdo->prepare("SELECT * FROM trip_agent_action_executions WHERE user_id=? AND dream_trip_id=? AND action_id IN ($marks)");$stmt->execute($params);$out=[];foreach($stmt->fetchAll()?:[] as $row)$out[(int)$row['action_id']]=$this->publicExecution($row);return $out;
    }
    ...
    public function editProposal(int $userId,int $tripId,int $actionId,array $input): array
    {
        ...
        $proposal['title']=$this->clip((string)($input['title']??$proposal['title']??''),180);if($proposal['title']==='')throw new InvalidArgumentException('Proposal title is required.');$proposal['notes']=$this->clip((string)($input['notes']??$proposal['notes']??''),1500);
        $price=trim((string)($input['price']??''));$proposal['price']=$price===''?null:max(0,(float)$price);$date=trim((string)($input['scheduled_date']??''));$proposal['scheduled_date']=$this->validDate($date);$daypart=(string)($input['daypart']??'');$proposal['daypart']=in_array($daypart,['morning','afternoon','evening','anytime'],true)?$daypart:null;
        $json=$this->json($proposal);$this->pdo->prepare('UPDATE trip_agent_action_executions SET proposal_json=?,updated_at=NOW() WHERE id=? AND user_id=? AND dream_trip_id=? AND status=\'awaiting_approval\'')->execute([$json,(int)$row['id'],$userId,$tripId]);$this->event((int)$row['id'],$actionId,$userId,$tripId,'proposal_edited',['title'=>$proposal['title']]);return $this->executionForAction($userId,$tripId,$actionId)??[];
    }

    public function approveAndApply(int $userId,int $tripId,int $actionId,array $input=[]): array
    {
        $this->requireReady();if($input)$this->editProposal($userId,$tripId,$actionId,$input);
        $this->pdo->beginTransaction();
        try{
            $stmt=$this->pdo->prepare("SELECT * FROM trip_agent_action_executions WHERE action_id=? AND user_id=? AND dream_trip_id=? FOR UPDATE");$stmt->execute([$actionId,$userId,$tripId]);$row=$stmt->fetch();if(!$row||$row['status']!=='awaiting_approval')throw new DomainException('This proposal is no longer waiting for approval.');$proposal=json_decode((string)($row['proposal_json']??''),true);if(!is_array($proposal))throw new RuntimeException('Proposal data is unavailable.');$executionId=(int)$row['id'];
            $this->pdo->prepare("UPDATE trip_agent_action_executions SET status='applied',approved_at=NOW(),applied_at=NOW(),error_message=NULL WHERE id=?")->execute([$executionId]);$this->event($executionId,$actionId,$userId,$tripId,'approved',['proposal_type'=>$row['proposal_type']]);
            $this->applyProposal($userId,$tripId,(string)$row['proposal_type'],$proposal);
    ...
    {
        $row=$this->rawExecution($userId,$tripId,$actionId);if(!$row||!in_array((string)$row['status'],['failed','rejected','cancelled'],true))throw new DomainException('This execution is not retryable.');return $this->acceptAndStart($userId,$tripId,$actionId);
    }

    private function dispatchExecution(int $executionId): array
    {
        $stmt=$this->pdo->prepare("SELECT e.*,a.title,a.body,a.action_kind,a.target_tab,a.suggestion_key,a.status action_status FROM trip_agent_action_executions e JOIN trip_agent_actions a ON a.id=e.action_id WHERE e.id=? LIMIT 1");$stmt->execute([$executionId]);$row=$stmt->fetch();if(!$row||$row['status']!=='queued')return $row?$this->publicExecution($row):[];$userId=(int)$row['user_id'];$tripId=(int)$row['dream_trip_id'];$jobService=new TripAgentJobService($this->pdo);if($jobService->hasActiveTripJobs($userId,$tripId))return $this->publicExecution($row);
        $agent=$this->agent((string)$row['agent_type']);$request=$this->executionRequest($row);$job=$jobService->enqueue($userId,$tripId,$agent,$request);$jobId=(int)($job['id']??0);if($jobId<1)throw new RuntimeException('Unable to queue the specialist execution job.');$this->pdo->prepare("UPDATE trip_agent_action_executions SET agent_job_id=?,status='agent_working',started_at=NOW(),error_message=NULL,updated_at=NOW() WHERE id=? AND status='queued'")->execute([$jobId,$executionId]);$this->event($executionId,(int)$row['action_id'],$userId,$tripId,'agent_working',['agent_type'=>$agent,'job_id'=>$jobId]);return $this->executionForAction($userId,$tripId,(int)$row['action_id'])??[];
    }

    private function createProposal(int $executionId,string $resultJson): void
    {
        $stmt=$this->pdo->prepare("SELECT e.*,a.title,a.body,a.action_kind,a.target_tab,a.suggestion_key,a.metadata_json action_metadata FROM trip_agent_action_executions e JOIN trip_agent_actions a ON a.id=e.action_id WHERE e.id=? LIMIT 1");$stmt->execute([$executionId]);$row=$stmt->fetch();if(!$row||$row['status']!=='agent_working')return;$decoded=json_decode($resultJson,true);$agentOutput=$this->clip((string)($decoded['message']??''),6000);$proposal=$this->proposal($row,$agentOutput);$json=$this->json($proposal['data']);$this->pdo->prepare("UPDATE trip_agent_action_executions SET status='awaiting_approval',proposal_type=?,proposal_json=?,agent_output=?,proposed_at=NOW(),error_message=NULL,updated_at=NOW() WHERE id=? AND status='agent_working'")->execute([$proposal['type'],$json,$agentOutput?:null,$executionId]);$this->event($executionId,(int)$row['action_id'],(int)$row['user_id'],(int)$row['dream_trip_id'],'proposal_ready',['proposal_type'=>$proposal['type'],'title'=>$proposal['data']['title']]);try{(new NotificationService($this->pdo))->create((int)$row['user_id'],'trip_action_approval','Trip change ready for approval',(string)$proposal['data']['title'],app_url('dream-trip.php?id='.(int)$row['dream_trip_id'].'&tab=overview#next-moves'),null,null,'trip-action-execution:'.$executionId.':proposal');}catch(Throwable $e){}
    }
    ...
    private function proposal(array $row,string $agentOutput): array
    {
        ...
        if($agent==='flights'||str_contains($key,'fare')||str_contains($key,'flight')){$type='booking_handoff';$itemType='flight';$external=true;}
        elseif($key==='lodging-partner'||str_contains($key,'lodging')){$type='booking_handoff';$itemType='hotel';$external=true;}
        elseif($agent==='events'){$type='itinerary_item';$itemType='activity';}
        elseif($agent==='local'){$type='itinerary_item';$itemType=(preg_match('/restaurant|dining|food|cafe|bar/i',$title.' '.$body)?'food':'activity');}
        elseif($agent==='itinerary'){$type='itinerary_item';$itemType='experience';}
        elseif($agent==='weather'){$type='itinerary_item';$itemType='idea';}
        elseif($agent==='budget'||$kind==='budget'){$type='budget_review';$itemType='idea';}
    ...
    {
        $proposal=json_decode((string)($row['proposal_json']??''),true);if(!is_array($proposal))$proposal=[];return ['id'=>(int)$row['id'],'action_id'=>(int)$row['action_id'],'trip_id'=>(int)$row['dream_trip_id'],'job_id'=>(int)($row['agent_job_id']??0)?:null,'agent_type'=>(string)$row['agent_type'],'status'=>(string)$row['status'],'proposal_type'=>(string)($row['proposal_type']??''),'proposal'=>$proposal,'agent_output'=>(string)($row['agent_output']??''),'error'=>(string)($row['error_message']??''),'queued_at'=>$row['queued_at']??null,'started_at'=>$row['started_at']??null,'proposed_at'=>$row['proposed_at']??null,'approved_at'=>$row['approved_at']??null,'applied_at'=>$row['applied_at']??null,'completed_at'=>$row['completed_at']??null,'updated_at'=>(string)$row['updated_at']];
    }
    ...
    private function itemType(string $value): string
    {
        return in_array($value,['idea','hotel','food','activity','flight','experience','merch'],true)?$value:'idea';
    }
    ...
    {
        $json=json_encode($value,JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE|JSON_INVALID_UTF8_SUBSTITUTE);return $json===false?'{}':$json;
    }

    private function requireReady(): void
    {
        if(!$this->ready())throw new RuntimeException('Run System Upgrade before using Trip Action execution and approval.');
    }
