...
final class BookingMailboxAgentContextService
{
    ...
    public function context(int $userId,int $limit=8): string
    {
        ...
        return 'CONNECTED BOOKING INBOX STATE (saved normalized change ledger only; no mailbox/provider refresh): '.implode('; ',$parts).'. Needs review means Vacation Brain has not changed the canonical reservation yet. Applied means the user reviewed and accepted an email-reported factual update; email sender identity is not provider verification. This context excludes OAuth tokens, mailbox source, account/sender identity, confirmation codes, traveler names, booking notes, and payment data. Chat never scans Gmail or applies a reservation change.';
    }
}
