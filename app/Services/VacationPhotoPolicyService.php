...
final class VacationPhotoPolicyService
{
    ...
    public function limits(int $userId): array
    {
        ...
        $stmt=$this->pdo->prepare('SELECT daily_limit,monthly_limit,lifetime_limit,comped_generations FROM vacation_photo_user_limits WHERE user_id=? LIMIT 1');
    ...
    public function usage(int $userId): array
    {
        ...
        return [
            ...
            'lifetime'=>(int)($row['lifetime_count']??0),
    ...
    public function status(int $userId): array
    {
        ...
        foreach(['daily','monthly','lifetime'] as $period){
            if($allowed&&$limits[$period]>0&&$remaining[$period]!==null&&$remaining[$period]<=0){
                $allowed=false;
                $reason=match($period){
                    'daily'=>'Your Vacation Yourself daily generation limit has been reached.',
                    'monthly'=>'Your Vacation Yourself monthly generation limit has been reached.',
                    default=>'Your Vacation Yourself generation allowance has been reached.',
                };
            }
        }
    ...
    public function assertCanGenerate(int $userId): void
    {
        ...
        if(!$status['allowed'])throw new RuntimeException((string)$status['reason']);
    }

    public function estimate(string $quality,string $size,int $referenceCount=0): float
    {
        $quality=in_array($quality,['low','medium','high'],true)?$quality:'medium';
        $shape=$size==='1024x1024'?'square':'large';
        $fallback=match($quality){
            'low'=>$shape==='square'?'0.0150':'0.0200',
            'high'=>$shape==='square'?'0.1500':'0.2200',
            default=>$shape==='square'?'0.0450':'0.0600',
        };
        $base=max(0,(float)site_setting('vacation_photos.estimate.'.$quality.'.'.$shape,$fallback));
    ...
    public function estimateBasis(string $quality,string $size,int $referenceCount=0): array
    {
        $quality=in_array($quality,['low','medium','high'],true)?$quality:'medium';
        $shape=$size==='1024x1024'?'square':'large';
    ...
    {
        if(array_key_exists('estimated_cost_usd',$row)&&$row['estimated_cost_usd']!==null)return round(max(0,(float)$row['estimated_cost_usd']),4);
        $refs=json_decode((string)($row['source_refs_json']??'[]'),true);
        return $this->estimate((string)($row['quality']??'medium'),(string)($row['size']??'1024x1024'),is_array($refs)?count($refs):0);
    }

    public function aggregate(?int $days=30): array
    {
        $where='WHERE g.status="completed"';$args=[];
        if($days!==null&&$days>0){
            $cutoff=(new DateTimeImmutable('now'))->modify('-'.max(1,$days).' days')->format('Y-m-d H:i:s');
            $where.=' AND g.created_at>=?';$args[]=$cutoff;
        }
        $stmt=$this->pdo->prepare('SELECT g.*,u.display_name,u.email FROM vacation_photo_generations g JOIN users u ON u.id=g.user_id '.$where.' ORDER BY g.created_at DESC LIMIT 5000');
        $stmt->execute($args);$rows=$stmt->fetchAll()?:[];$total=0.0;$retained=0;$deleted=0;$byQuality=['low'=>0,'medium'=>0,'high'=>0];
        foreach($rows as &$row){
            $row['_estimated_cost']=$this->generationEstimate($row);$total+=$row['_estimated_cost'];
            $isRetained=empty($row['deleted_at'])&&!empty($row['image_url']);$row['_retained']=$isRetained;
            if($isRetained)$retained++;else $deleted++;
            $quality=(string)($row['quality']??'medium');if(isset($byQuality[$quality]))$byQuality[$quality]++;
        }
        unset($row);
        return ['rows'=>$rows,'count'=>count($rows),'retained_count'=>$retained,'deleted_count'=>$deleted,'estimated_cost'=>round($total,2),'by_quality'=>$byQuality];
    }

    public function saveDefaults(array $input,int $adminId): void
    {
        $settings=[
            'vacation_photos.enabled'=>!empty($input['enabled'])?'1':'0',
            'vacation_photos.gallery_enabled'=>!empty($input['gallery_enabled'])?'1':'0',
            'vacation_photos.sharing_enabled'=>!empty($input['sharing_enabled'])?'1':'0',
            'vacation_photos.daily_limit'=>(string)max(0,min(1000,(int)($input['daily_limit']??8))),
            'vacation_photos.monthly_limit'=>(string)max(0,min(10000,(int)($input['monthly_limit']??40))),
            'vacation_photos.lifetime_limit'=>(string)max(0,min(1000000,(int)($input['lifetime_limit']??0))),
            'vacation_photos.default_quality'=>in_array(($input['default_quality']??'medium'),['low','medium','high'],true)?(string)$input['default_quality']:'medium',
            'vacation_photos.default_over_the_top'=>(string)max(0,min(100,(int)($input['default_over_the_top']??35))),
            'vacation_photos.max_reference_images'=>(string)max(1,min(4,(int)($input['max_reference_images']??4))),
        ];
        foreach(['low.square','low.large','medium.square','medium.large','high.square','high.large','reference_image'] as $key){
            $settings['vacation_photos.estimate.'.$key]=number_format(max(0,min(10,(float)($input['cost_'.$key]??site_setting('vacation_photos.estimate.'.$key,'0')))),4,'.','');
        }
