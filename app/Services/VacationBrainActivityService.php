...
final class VacationBrainActivityService
{
    ...
    private const SOURCE_WEIGHT=['agent'=>12,'intelligence'=>9,'watch_alert'=>15,'user_event'=>4,'trip'=>8,'destination'=>5];
    ...
    public function snapshot(int $userId,?int $tripId=null): array
    {
        ...
        if($tripId!==null&&!$this->ownsTrip($userId,$tripId))throw new OutOfBoundsException('Trip not found.');

        $trips=$this->tripStats($userId,$tripId);
        $destinations=$tripId===null?$this->destinationStats($userId):['selected'=>0,'watching_legacy'=>0,'last_at'=>null];
        ...
        foreach(self::CHANNELS as $channel){
            ...
            if($channel==='overview'){
                $score=8+$persistent+min(24,$agent24*7)+min(16,(int)$intel['day_total']*2)+min(16,(int)$watches['alerts_day']*6)+min(10,(int)$events['day']);
            }elseif($channel==='itinerary'){
                $score=5+$persistent+min(35,$agent24*9)+min(15,$agent7*2)+min(12,(int)$events['trip_actions_day']*3);
            ...
            $channels[$channel]=[
                ...
                'last_activity'=>$this->lastActivityForChannel($channel,$agents,$intel,$watches,$trips,$items,$events),
        ...
        if($tripId!==null){
            $overall=max($overall,(int)round(array_sum(array_map(static fn(array $row)=>(int)$row['intensity'],$channels))/max(1,count($channels))));
        }

        $overallSeries=[];
        for($i=0;$i<24;$i++){
            $sum=0;foreach($channels as $channel)$sum+=(int)($channel['series'][$i]??0);
            $overallSeries[]=(int)round($sum/max(1,count($channels)));
        }
        ...
        return [
            ...
            'stats'=>[
                ...
                'planning_actions_24h'=>(int)$events['trip_actions_day'],
    ...
    private function tripStats(int $userId,?int $tripId): array
    {
        ...
        $where='user_id=? AND status<>\'abandoned\'';$params=[$userId];
        if($tripId!==null){$where.=' AND id=?';$params[]=$tripId;}
        $stmt=$this->pdo->prepare("SELECT id,status,target_budget,booking_readiness,updated_at FROM dream_trips WHERE $where ORDER BY updated_at DESC,id DESC");
        $stmt->execute($params);$rows=$stmt->fetchAll()?:[];$withBudget=0;$statuses=[];
    ...
    private function destinationStats(int $userId): array
    {
        ...
        $stmt=$this->pdo->prepare('SELECT COUNT(*) selected_count,SUM(is_watching=1) watching_count,MAX(updated_at) last_at FROM dashboard_destination_context WHERE user_id=? AND is_selected=1');
    ...
    private function watchStats(int $userId,?int $tripId): array
    {
        ...
        foreach($rows as $row){
            foreach(['weather','flights','events','places'] as $type){$col='watch_'.$type;if(!empty($row[$col]))$out['signals'][$type]++;}
            $at=trim((string)($row['last_checked_at']??''));if($at!==''&&($out['last_at']===null||$at>$out['last_at']))$out['last_at']=$at;
        }
        if(db_table_exists('travel_watch_events')){
            ...
            $stmt=$this->pdo->prepare($sql);$stmt->execute($eventParams);
            foreach($stmt->fetchAll()?:[] as $row){$type=(string)$row['data_type'];$out['alerts_by_type'][$type]=(int)$row['c'];$out['alerts_day']+=(int)$row['c'];$out['last_by_type'][$type]=(string)$row['last_at'];if($out['last_at']===null||(string)$row['last_at']>$out['last_at'])$out['last_at']=(string)$row['last_at'];}
        }
        return $out;
    }
    ...
    {
        $out=['day_total'=>0,'week_total'=>0,'by_channel'=>[],'last_by_channel'=>[],'last_at'=>null];
        if(!db_table_exists('trip_agent_messages'))return $out;
        ...
        foreach($stmt->fetchAll()?:[] as $row){
            $channel=$this->normalizeChannel((string)$row['agent_type']);$day=(int)$row['day_count'];$week=(int)$row['week_count'];$last=(string)$row['last_at'];
            $bucket=$out['by_channel'][$channel]??['day'=>0,'week'=>0];$bucket['day']+=$day;$bucket['week']+=$week;$out['by_channel'][$channel]=$bucket;
            $out['day_total']+=$day;$out['week_total']+=$week;
            if(!isset($out['last_by_channel'][$channel])||$last>$out['last_by_channel'][$channel])$out['last_by_channel'][$channel]=$last;
            if($out['last_at']===null||$last>$out['last_at'])$out['last_at']=$last;
        }
        return $out;
    }
    ...
    private function intelligenceStats(int $userId,?int $tripId): array
    {
        ...
        foreach($stmt->fetchAll()?:[] as $row){$type=(string)$row['data_type'];$day=(int)$row['day_count'];$week=(int)$row['week_count'];$last=(string)$row['last_at'];$out['by_type'][$type]=['day'=>$day,'week'=>$week];$out['day_total']+=$day;$out['week_total']+=$week;$out['last_by_type'][$type]=$last;if($out['last_at']===null||$last>$out['last_at'])$out['last_at']=$last;}
        return $out;
    }
    ...
    private function itemStats(int $userId,?int $tripId): array
    {
        ...
        $stmt=$this->pdo->prepare("SELECT COUNT(dti.id) total,$scheduled scheduled,$last last_at FROM dream_trips dt LEFT JOIN dream_trip_items dti ON dti.dream_trip_id=dt.id WHERE $where");$stmt->execute($params);$row=$stmt->fetch()?:[];
        return ['total'=>(int)($row['total']??0),'scheduled'=>(int)($row['scheduled']??0),'last_at'=>$row['last_at']??null];
    }

    private function userEventStats(int $userId,?int $tripId): array
    {
        $out=['day'=>0,'week'=>0,'trip_actions_day'=>0,'last_at'=>null];
        if(!db_table_exists('user_events')||!db_column_exists('user_events','occurred_at'))return $out;
        $where='user_id=? AND occurred_at>=DATE_SUB(NOW(),INTERVAL 7 DAY)';$params=[$userId];
        if($tripId!==null&&db_column_exists('user_events','dream_trip_id')){$where.=' AND dream_trip_id=?';$params[]=$tripId;}
        $stmt=$this->pdo->prepare("SELECT SUM(occurred_at>=DATE_SUB(NOW(),INTERVAL 1 DAY)) day_count,COUNT(*) week_count,SUM(occurred_at>=DATE_SUB(NOW(),INTERVAL 1 DAY) AND event_type IN ('dream_trip_created','dream_trip_viewed','dream_item_added')) trip_actions,MAX(occurred_at) last_at FROM user_events WHERE $where");
    ...
    private function recentSignals(int $userId,?int $tripId): array
    {
        ...
        if(db_table_exists('trip_agent_messages')){
            ...
            $stmt=$this->pdo->prepare("SELECT agent_type channel,created_at at,body detail,dream_trip_id trip_id FROM trip_agent_messages WHERE $where ORDER BY created_at DESC LIMIT 120");$stmt->execute($params);
            foreach($stmt->fetchAll()?:[] as $row){$channel=$this->normalizeChannel((string)$row['channel']);$rows[]=['channel'=>$channel,'at'=>(string)$row['at'],'kind'=>'agent','weight'=>self::SOURCE_WEIGHT['agent'],'title'=>$this->channelLabel($channel).' worked','detail'=>$this->clip((string)$row['detail'],180),'trip_id'=>(int)$row['trip_id']];}
        }
        if(db_table_exists('trip_intelligence_snapshots')){
            ...
            $stmt=$this->pdo->prepare("SELECT data_type,provider,source_status,observed_at at,dream_trip_id trip_id FROM trip_intelligence_snapshots WHERE $where ORDER BY observed_at DESC LIMIT 120");$stmt->execute($params);
            foreach($stmt->fetchAll()?:[] as $row){$channel=$this->normalizeChannel((string)$row['data_type']);$rows[]=['channel'=>$channel,'at'=>(string)$row['at'],'kind'=>'intelligence','weight'=>self::SOURCE_WEIGHT['intelligence'],'title'=>$this->channelLabel($channel).' refreshed','detail'=>trim((string)$row['provider']).' · '.((string)$row['source_status']==='success'?'fresh data':'provider issue'),'trip_id'=>(int)$row['trip_id']];}
        }
        if(db_table_exists('travel_watch_events')){
            $sql='SELECT e.data_type,e.title,e.body,e.created_at at,w.dream_trip_id trip_id FROM travel_watch_events e JOIN travel_watches w ON w.id=e.watch_id WHERE e.user_id=? AND e.created_at>=?';$params=[$userId,$cutoff];if($tripId!==null){$sql.=' AND w.dream_trip_id=?';$params[]=$tripId;}$sql.=' ORDER BY e.created_at DESC LIMIT 80';
            $stmt=$this->pdo->prepare($sql);$stmt->execute($params);
            foreach($stmt->fetchAll()?:[] as $row){$channel=$this->normalizeChannel((string)$row['data_type']);$rows[]=['channel'=>$channel,'at'=>(string)$row['at'],'kind'=>'watch_alert','weight'=>self::SOURCE_WEIGHT['watch_alert'],'title'=>(string)$row['title'],'detail'=>$this->clip((string)$row['body'],180),'trip_id'=>(int)($row['trip_id']??0)];}
        }
        if(db_table_exists('user_events')&&db_column_exists('user_events','occurred_at')){
            $where='user_id=? AND occurred_at>=? AND event_type IN (\'dream_trip_created\',\'dream_trip_viewed\',\'dream_item_added\')';$params=[$userId,$cutoff];
            if($tripId!==null&&db_column_exists('user_events','dream_trip_id')){$where.=' AND dream_trip_id=?';$params[]=$tripId;}
            $stmt=$this->pdo->prepare("SELECT event_type,value_text,occurred_at at,dream_trip_id trip_id FROM user_events WHERE $where ORDER BY occurred_at DESC LIMIT 100");$stmt->execute($params);
            foreach($stmt->fetchAll()?:[] as $row){$signal=$this->userEventSignal($row);if($signal!==null)$rows[]=$signal;}
        }
        if($tripId===null&&db_table_exists('dashboard_destination_context')){
            $stmt=$this->pdo->prepare('SELECT destination_name,updated_at at FROM dashboard_destination_context WHERE user_id=? AND is_selected=1 AND updated_at>=? ORDER BY updated_at DESC LIMIT 40');$stmt->execute([$userId,$cutoff]);
            foreach($stmt->fetchAll()?:[] as $row)$rows[]=['channel'=>'overview','at'=>(string)$row['at'],'kind'=>'destination','weight'=>self::SOURCE_WEIGHT['destination'],'title'=>'Destination selected','detail'=>(string)$row['destination_name'],'trip_id'=>0];
        }
        usort($rows,static fn($a,$b)=>strcmp((string)$b['at'],(string)$a['at']));return $rows;
    }

    private function userEventSignal(array $row): ?array
    {
        $type=(string)($row['event_type']??'');$tripId=(int)($row['trip_id']??0);$detail=$this->clip((string)($row['value_text']??''),180);$at=(string)($row['at']??'');
        return match($type){
            'dream_trip_created'=>['channel'=>'overview','at'=>$at,'kind'=>'trip','weight'=>self::SOURCE_WEIGHT['trip'],'title'=>'Trip planned','detail'=>$detail!==''?$detail:'A new trip entered the planning brain.','trip_id'=>$tripId],
            'dream_trip_viewed'=>['channel'=>'overview','at'=>$at,'kind'=>'user_event','weight'=>3,'title'=>'Trip revisited','detail'=>$detail!==''?$detail:'Trip planning reopened.','trip_id'=>$tripId],
            'dream_item_added'=>['channel'=>'itinerary','at'=>$at,'kind'=>'user_event','weight'=>8,'title'=>'Itinerary updated','detail'=>$detail!==''?ucfirst($detail).' item added':'A trip item was added.','trip_id'=>$tripId],
            default=>null,
        };
    }
    ...
    {
        $now=time();$buckets=array_fill(0,24,0);
        foreach($signals as $signal){
            if(($signal['channel']??'')!==$channel&&$channel!=='overview')continue;
            $ts=strtotime((string)($signal['at']??''));if(!$ts)continue;$age=$now-$ts;if($age<0||$age>=86400)continue;
            $index=23-(int)floor($age/3600);if($index<0||$index>23)continue;$weight=(int)($signal['weight']??5);
            if($channel==='overview'&&($signal['channel']??'')!=='overview')$weight=(int)round($weight*.45);
            $buckets[$index]=min(100,$buckets[$index]+$weight);
        }
        $carry=0;for($i=0;$i<24;$i++){if($buckets[$i]>0)$carry=max($carry,$buckets[$i]);else$carry=(int)floor($carry*.62);$buckets[$i]=min(100,$buckets[$i]+(int)floor($carry*.22));}
        return $buckets;
    }

    private function lastActivityForChannel(string $channel,array $agents,array $intel,array $watches,array $trips,array $items,array $events): ?string
    {
        $candidates=[];
        if(!empty($agents['last_by_channel'][$channel]))$candidates[]=$agents['last_by_channel'][$channel];
        $intelKey=$channel==='local'?'places':$channel;if(!empty($intel['last_by_type'][$intelKey]))$candidates[]=$intel['last_by_type'][$intelKey];
        $watchKey=$channel==='local'?'places':$channel;if(!empty($watches['last_by_type'][$watchKey]))$candidates[]=$watches['last_by_type'][$watchKey];
        if($channel==='overview'){foreach([$trips['last_at']??null,$watches['last_at']??null,$events['last_at']??null] as $at)if($at)$candidates[]=$at;}
        if($channel==='itinerary'&&!empty($items['last_at']))$candidates[]=$items['last_at'];
        if(!$candidates)return null;rsort($candidates);return (string)$candidates[0];
    }
    ...
    private function summary(int $score,array $trips,array $destinations,array $watches,array $agents,array $intel): string
    {
        ...
        $parts=[];
        if((int)$trips['active']>0)$parts[]=(int)$trips['active'].' active trip'.((int)$trips['active']===1?'':'s');
        if((int)$watches['active']>0)$parts[]=(int)$watches['active'].' watch'.((int)$watches['active']===1?'':'es');
        if((int)$agents['day_total']>0)$parts[]=(int)$agents['day_total'].' agent result'.((int)$agents['day_total']===1?'':'s').' today';
        if((int)$intel['day_total']>0)$parts[]=(int)$intel['day_total'].' data refresh'.((int)$intel['day_total']===1?'':'es');
        if(!$parts&&(int)$destinations['selected']>0)$parts[]=(int)$destinations['selected'].' selected destination'.((int)$destinations['selected']===1?'':'s');
        return 'The brain is processing '.implode(', ',$parts).'.';
    }
    ...
    {
        if($tripId&&$tripId>0)return app_url('dream-trip.php?id='.$tripId.'&tab='.rawurlencode($channel));return app_url('dream.php');
    }
