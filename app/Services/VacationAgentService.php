...
final class VacationAgentService
{
    ...
    private function reply(int $userId,string $message): string
    {
        ...
        if($this->isCollaborationQuestion($q)){
            try{
                if(class_exists('TripCollaborationAgentContextService')){
                    $rows=(new TripCollaborationAgentContextService($this->pdo))->fallback($userId);
                    if($rows){$answer=$this->collaborationFallback($rows);if($answer!=='')return $answer;}
                }
            }catch(Throwable){}
            return 'I do not have an active shared-trip membership to summarize. Open Trips to see invitations you have already accepted. Collaboration state is saved locally and this chat does not expose invitation emails or anyone else’s private travel profile.';
        }
    ...
    private function isCollaborationQuestion(string $q): bool
    {
        foreach(['shared trip','trip members','trip travelers','who is going','who’s going','whos going','my rsvp','trip rsvp','co-planner','co planner','trip vote','itinerary vote','travelers invited','collaborative trip'] as $needle)if(str_contains($q,$needle))return true;
        return false;
    }

    private function collaborationFallback(array $rows): string
    {
        $row=$rows[0]??null;if(!is_array($row))return '';$trip=(string)($row['name']??'Shared trip');$role=ucwords(str_replace('_',' ',(string)($row['role']??'traveler')));$members=(int)($row['member_count']??0);$going=(int)($row['going_count']??0);$votes=(int)($row['vote_count']??0);$rsvp=(string)($row['rsvp']??'unknown');
        $answer=$trip.' has '.$members.' people in the shared workspace; '.$going.' are marked going. Your role is '.$role.'.';if((string)($row['role']??'')!=='owner')$answer.=' Your RSVP is '.str_replace('_',' ',$rsvp).'.';if($votes>0)$answer.=' The shared itinerary has '.$votes.' saved vote'.($votes===1?'':'s').'.';$answer.=' Open the shared trip workspace to RSVP, vote, or plan within your role. I did not read collaborator email addresses or anyone else’s private Traveler Memory.';return $answer;
    }
    ...
    {
        foreach(['flight status','my flight','gate','delay','weather for my trip','weather on my trip','hotel price','lodging','hotel availability','what is happening near','events on my trip','trip weather','what should i worry','needs attention','anything changed','what changed','trip risk','trip conflict','proactive','local concierge','what should we do nearby','what should we do near','what should we do tonight','what can we do tonight','what is good nearby','where should we eat tonight','what should we eat nearby'] as $needle)if(str_contains($q,$needle))return true;return false;
    }

    private function liveFallback(string $q,array $live): string
    {
        $trip=$live['trip']??[];$tripName=(string)($trip['name']??'your trip');
        $localQuestion=false;foreach(['local concierge','what should we do nearby','what should we do near','what should we do tonight','what can we do tonight','what is good nearby','where should we eat tonight','what should we eat nearby','what is happening near'] as $needle)if(str_contains($q,$needle)){$localQuestion=true;break;}
        if($localQuestion){$run=$live['concierge']??null;if(is_array($run)){$suggestions=[];foreach((array)($run['suggestions']??[]) as $s){if(!is_array($s)||!empty($s['dismissed']))continue;$suggestions[]=$s;if(count($suggestions)>=3)break;}if($suggestions){$bits=[];foreach($suggestions as $s)$bits[]=(string)($s['title']??'Local option').' ['.(string)($s['category']??'local').']';return 'From your last saved Local Concierge run around '.(string)($run['anchor_label']??'the trip').': '.implode('; ',$bits).'. This chat did not refresh your location or any provider, and these are time-sensitive suggestions—not reservations. Open Local Concierge to refresh or verify the linked sources.';}return 'Your last saved Local Concierge run has no active suggestions left. Open Local Concierge to explicitly refresh the trip destination or use your location once. This chat did not request device location or contact a provider.';}return 'I do not have a saved Local Concierge run yet. Open Local Concierge from the trip and explicitly choose the trip destination or “Use my location once.” Chat itself will not request or refresh device location.';}
