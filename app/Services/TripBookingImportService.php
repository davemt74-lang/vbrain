...
final class TripBookingImportService
{
    ...
    private function parseLocal(string $text): array
    {
        ...
        if(preg_match('/\b(USD|EUR|GBP|CAD|AUD|JPY)\b/i',$flat,$m))$out['currency']=strtoupper($m[1]);elseif(str_contains($flat,'$'))$out['currency']='USD';$out=array_merge($out,$this->extractDates($text));
    ...
    {
        $out=[];$patterns=['/\b20\d{2}[-\/]\d{1,2}[-\/]\d{1,2}(?:[ T]\d{1,2}:\d{2}(?:\s*[AP]M)?)?/i','/\b(?:Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:tember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)\s+\d{1,2}(?:,?\s+20\d{2})(?:\s+(?:at\s+)?\d{1,2}:\d{2}\s*(?:AM|PM)?)?/i','/\b\d{1,2}\/\d{1,2}\/20\d{2}(?:\s+\d{1,2}:\d{2}\s*(?:AM|PM)?)?/i'];$dates=[];foreach($patterns as $p){if(preg_match_all($p,$text,$m)){foreach($m[0] as $raw){$ts=strtotime($raw);if($ts)$dates[$ts]=date('Y-m-d H:i:s',$ts);}}}ksort($dates);$vals=array_values($dates);if($vals)$out['starts_at']=$vals[0];if(count($vals)>1)$out['ends_at']=$vals[count($vals)-1];
        if(preg_match('/(?:cancel(?:lation)?(?:\s+free)?(?:\s+by|\s+before)?)[^\r\n]{0,40}?((?:20\d{2}[-\/]\d{1,2}[-\/]\d{1,2})|(?:[A-Za-z]{3,9}\s+\d{1,2},?\s+20\d{2})|(?:\d{1,2}\/\d{1,2}\/20\d{2}))/i',$text,$m)&&($ts=strtotime($m[1])))$out['cancellation_deadline']=date('Y-m-d H:i:s',$ts);
        if(preg_match('/check[- ]?in(?:\s+opens|\s+from|\s+at)?[^\r\n]{0,35}?((?:20\d{2}[-\/]\d{1,2}[-\/]\d{1,2})|(?:[A-Za-z]{3,9}\s+\d{1,2},?\s+20\d{2}))(?:\s+(?:at\s+)?(\d{1,2}:\d{2}\s*(?:AM|PM)?))?/i',$text,$m)&&($ts=strtotime(trim($m[1].' '.($m[2]??'')))))$out['checkin_opens_at']=date('Y-m-d H:i:s',$ts);return $out;
    }
    ...
    private function autoMaterializeEligible(array $p): bool{return (string)($p['booking_type']??'other')!=='other'&&(!empty($p['flight_number'])||!empty($p['starts_at'])||!empty($p['provider_name']));}
