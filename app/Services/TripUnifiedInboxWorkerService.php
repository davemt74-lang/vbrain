...
final class TripUnifiedInboxWorkerService
{
    ...
    public function runDue(int $limit=20): array
    {
        ...
        $sql="SELECT c.user_id
            ...
            LIMIT {$limit}";
        $rows=$this->pdo->query($sql)->fetchAll(PDO::FETCH_COLUMN)?:[];$out=['users'=>0,'items'=>0,'errors'=>0];$inbox=new TripUnifiedInboxService($this->pdo);
        foreach($rows as $uid){$uid=(int)$uid;$out['users']++;try{$r=$inbox->syncUser($uid);$out['items']+=(int)($r['items']??0);}catch(Throwable $e){$out['errors']++;error_log('Unified Trip Inbox worker sync failed for user '.$uid.': '.$e->getMessage());}finally{$this->markSynced($uid);}}
        return $out;
    }
