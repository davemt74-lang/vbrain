 ...
 *
 * Combines shared target/category budgets and canonical booking totals with owner-only
...
final class TripCostIntelligenceService
{
    ...
    public function snapshot(int $userId,int $tripId): array
    {
        ...
        $owner=!empty($access['is_owner']);$canPlan=!empty($access['can_plan']);
        $bookings=$this->bookingRows($tripId);$bookingAgg=$this->aggregateBookings($bookings);
        $plans=$canPlan?$this->planRows($tripId):[];
        $shared=['plans'=>$plans,'bookings'=>$bookingAgg,'target_budget'=>$canPlan&&$trip['target_budget']!==null?(float)$trip['target_budget']:null,'currency'=>(string)($trip['currency']??'USD')];
        if(!$owner){
            return ['ready'=>true,'trip'=>$this->publicTrip($trip),'role'=>$role,'can_manage'=>false,'can_view_shared_budget'=>$canPlan,'can_view_private_costs'=>false,'categories'=>self::CATEGORIES,'shared'=>$shared,'economics'=>[],'manual_entries'=>[],'history_model'=>[],'privacy_note'=>'Shared collaborators receive only the budget/booking view allowed by their trip role. Manual expenses, Trip Memory actual spend, disruption reimbursements and learned cost history are owner-only.','safety_note'=>$this->safetyNote()];
        }
        $economics=$this->ownerEconomics($userId,$trip,$bookings,$plans);
    ...
    {
        if(!$this->ready()||!db_table_exists('trip_memories'))return ['trip_count'=>0,'currencies'=>[]];$limit=max(1,min(80,$limit));
        $q=$this->pdo->prepare("SELECT m.dream_trip_id FROM trip_memories m JOIN dream_trips dt ON dt.id=m.dream_trip_id AND dt.user_id=m.user_id WHERE m.user_id=? AND m.status='complete' AND m.learning_enabled=1 ORDER BY COALESCE(m.completed_at,dt.end_date,dt.updated_at) DESC,m.id DESC LIMIT {$limit}");$q->execute([$userId]);$ids=array_map('intval',$q->fetchAll(PDO::FETCH_COLUMN)?:[]);$groups=[];$used=0;
        foreach($ids as $tripId){try{$trip=$this->trip($tripId);$econ=$this->ownerEconomics($userId,$trip,$this->bookingRows($tripId),$this->planRows($tripId));}catch(Throwable){continue;}foreach((array)($econ['by_currency']??[]) as $currency=>$row){$basis=$row['comparison_basis']??null;if($basis===null||(float)$basis<=0)continue;$used++;if(!isset($groups[$currency]))$groups[$currency]=['samples'=>0,'total_cost'=>0.0,'total_budget_variance_pct'=>0.0,'budget_samples'=>0,'total_per_day'=>0.0,'per_day_samples'=>0,'total_per_traveler_day'=>0.0,'ptd_samples'=>0,'disruption_cost'=>0.0,'cash_recovered'=>0.0,'gross_cost'=>0.0,'category_gross'=>[]];$g=&$groups[$currency];$g['samples']++;$g['total_cost']+=(float)$basis;if($row['budget_variance_pct']!==null){$g['total_budget_variance_pct']+=(float)$row['budget_variance_pct'];$g['budget_samples']++;}if($row['per_day']!==null){$g['total_per_day']+=(float)$row['per_day'];$g['per_day_samples']++;}if($row['per_traveler_day']!==null){$g['total_per_traveler_day']+=(float)$row['per_traveler_day'];$g['ptd_samples']++;}$g['disruption_cost']+=(float)$row['disruption_cost'];$g['cash_recovered']+=(float)$row['cash_recovered'];$g['gross_cost']+=(float)$row['structured_gross_cash_outflow'];foreach((array)($econ['categories'][$currency]??[]) as $cat=>$c)$g['category_gross'][$cat]=($g['category_gross'][$cat]??0)+(float)($c['gross_cost']??0);unset($g);}}
        ...
        return ['trip_count'=>count($ids),'economics_samples'=>$used,'currencies'=>$out,'learning_note'=>'Cost history uses completed Trip Memories with learning enabled. Currencies stay separate and no exchange rate is guessed.'];
    }
    ...
    public function safeAgentContext(int $userId,int $limitTrips=3): array
    {
        ...
        $out=[];foreach(array_values(array_unique($ids)) as $tripId){try{$access=$this->access($userId,$tripId);$trip=$this->trip($tripId);$book=$this->aggregateBookings($this->bookingRows($tripId));}catch(Throwable){continue;}$item=['trip_id'=>$tripId,'trip_name'=>(string)$trip['name'],'role'=>(string)($access['role']??'viewer'),'currency'=>(string)($trip['currency']??'USD'),'booking_totals'=>$book['by_currency']??[]];if(!empty($access['can_plan']))$item['target_budget']=$trip['target_budget']!==null?(float)$trip['target_budget']:null;if(!empty($access['is_owner'])){$e=$this->ownerEconomics($userId,$trip,$this->bookingRows($tripId),$this->planRows($tripId));$item['economics']=$e['by_currency']??[];}else $item['economics']=[];$out[]=$item;}
        return $out;
    }

    private function ownerEconomics(int $userId,array $trip,array $bookings,array $plans): array
    {
        ...
        foreach((array)($bookingAgg['rows']??[]) as $r){$c=$this->currency((string)$r['currency']);$cat=$this->bookingCategory((string)$r['booking_type']);$ensure($c,$cat);$cats[$c][$cat]['booking_cost']+=(float)$r['amount'];$by[$c]['booking_cost']+=(float)$r['amount'];}
        ...
        foreach($by as $currency=>&$row){$row['structured_gross_cash_outflow']=round($row['booking_cost']+$row['manual_cost']+$row['disruption_cost'],2);$row['cash_outstanding']=round(max(0,$row['cash_expected']-$row['cash_recovered']-$row['written_off']),2);$row['credit_outstanding']=round(max(0,$row['credits_expected']-$row['credits_received']),2);$row['net_cash_cost']=round($row['structured_gross_cash_outflow']-$row['cash_recovered'],2);$row['comparison_basis']=$row['reported_actual']!==null?round((float)$row['reported_actual'],2):$row['net_cash_cost'];$row['comparison_basis_source']=$row['reported_actual']!==null?'traveler_reported_actual':'structured_net_cash';if($row['target_budget']!==null){$row['budget_variance']=round($row['comparison_basis']-(float)$row['target_budget'],2);$row['budget_variance_pct']=(float)$row['target_budget']>0?round($row['budget_variance']/(float)$row['target_budget']*100,1):null;}else{$row['budget_variance']=null;$row['budget_variance_pct']=null;}$row['per_day']=$days>0?round($row['comparison_basis']/$days,2):null;$row['per_traveler']=round($row['comparison_basis']/$travelers,2);$row['per_traveler_day']=$days>0?round($row['comparison_basis']/($days*$travelers),2):null;$row['structured_coverage_pct']=$row['reported_actual']!==null&&(float)$row['reported_actual']>0?round(min(100,$row['structured_gross_cash_outflow']/(float)$row['reported_actual']*100),1):null;}unset($row);
        foreach($cats as $currency=>&$currencyCats)foreach($currencyCats as $cat=>&$r){$r['gross_cost']=round($r['booking_cost']+$r['manual_cost']+$r['disruption_cost'],2);$r['cash_outstanding']=round(max(0,$r['cash_expected']-$r['cash_recovered']-$r['written_off']),2);$r['credit_outstanding']=round(max(0,$r['credits_expected']-$r['credits_received']),2);$r['net_cash_cost']=round($r['gross_cost']-$r['cash_recovered'],2);foreach($r as $k=>$v)if(is_float($v))$r[$k]=round($v,2);}unset($r,$currencyCats);
        $forecast=$this->forecastFromModel($this->historyModelShallow($userId,25,$tripId),$tripCurrency,$days,$travelers);
        return ['by_currency'=>$by,'categories'=>$cats,'trip_days'=>$days,'travelers'=>$travelers,'forecast'=>$forecast,'memory_actual'=>$memory,'basis_note'=>'Budget variance uses traveler-reported Trip Memory actual spend when available; otherwise it uses structured net cash cost. Provider credits remain non-cash and never reduce net cash cost.','currency_note'=>'Each currency is shown independently. Vacation Brain does not guess exchange rates.'];
    }

    private function historyModelShallow(int $userId,int $limit,int $excludeTripId): array
    {
        if(!db_table_exists('trip_memories'))return ['currencies'=>[]];$q=$this->pdo->prepare("SELECT m.dream_trip_id,m.actual_spend,m.currency,dt.start_date,dt.end_date,dt.travelers,dt.target_budget FROM trip_memories m JOIN dream_trips dt ON dt.id=m.dream_trip_id AND dt.user_id=m.user_id WHERE m.user_id=? AND m.status='complete' AND m.learning_enabled=1 AND m.dream_trip_id<>? AND m.actual_spend IS NOT NULL ORDER BY COALESCE(m.completed_at,dt.end_date,dt.updated_at) DESC LIMIT {$limit}");$q->execute([$userId,$excludeTripId]);$g=[];foreach($q->fetchAll()?:[] as $r){$c=$this->currency((string)$r['currency']);$days=$this->daysFromDates($r['start_date']??null,$r['end_date']??null);$trav=max(1,(int)($r['travelers']??1));$amt=(float)$r['actual_spend'];$g[$c]??=['samples'=>0,'total'=>0.0,'ptd'=>0.0];$g[$c]['samples']++;$g[$c]['total']+=$amt;$g[$c]['ptd']+=$amt/max(1,$days*$trav);}foreach($g as &$r){$r['average_trip_cost']=round($r['total']/max(1,$r['samples']),2);$r['average_per_traveler_day']=round($r['ptd']/max(1,$r['samples']),2);}unset($r);return ['currencies'=>$g];
    }
    ...
    private function emptyCurrencyRow(): array{return ['target_budget'=>null,'category_plan'=>0.0,'booking_cost'=>0.0,'manual_cost'=>0.0,'disruption_cost'=>0.0,'structured_gross_cash_outflow'=>0.0,'cash_expected'=>0.0,'cash_recovered'=>0.0,'cash_outstanding'=>0.0,'credits_expected'=>0.0,'credits_received'=>0.0,'credit_outstanding'=>0.0,'written_off'=>0.0,'net_cash_cost'=>0.0,'reported_actual'=>null,'comparison_basis'=>null,'comparison_basis_source'=>'structured_net_cash','budget_variance'=>null,'budget_variance_pct'=>null,'per_day'=>null,'per_traveler'=>null,'per_traveler_day'=>null,'structured_coverage_pct'=>null];}
    private function emptyCategoryRow(): array{return ['planned'=>0.0,'booking_cost'=>0.0,'manual_cost'=>0.0,'disruption_cost'=>0.0,'linked_cost_reference'=>0.0,'linked_disruption_reference'=>0.0,'gross_cost'=>0.0,'cash_expected'=>0.0,'cash_recovered'=>0.0,'cash_outstanding'=>0.0,'credits_expected'=>0.0,'credits_received'=>0.0,'credit_outstanding'=>0.0,'written_off'=>0.0,'net_cash_cost'=>0.0];}
