...
try{
    if(!$service->ready())throw new RuntimeException('Run System Upgrade before using live agent jobs.');
    ...
    if($action==='enqueue'){
        $job=$service->enqueue($userId,$tripId,(string)($_POST['agent_type']??'overview'),(string)($_POST['message']??''));echo json_encode(['ok'=>true,'job'=>$job],JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE);exit;
    }
    if($action==='run_all'){
        $jobs=$service->enqueueAll($userId,$tripId);echo json_encode(['ok'=>true,'jobs'=>$jobs],JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE);exit;
    }
    if($action==='cancel'){
        $job=$service->cancel($userId,(int)($_POST['job_id']??0));echo json_encode(['ok'=>true,'job'=>$job],JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE);exit;
    }
