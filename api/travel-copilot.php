...
$userId=require_auth();$tripId=(int)($_GET['trip_id']??$_POST['trip_id']??0);$date=(string)($_GET['date']??$_POST['date']??'');$pdo=db();$service=new TripTravelDayCopilotService($pdo);
try{
    ...
    verify_csrf();
    $access=(new TripItineraryIntelligenceService($pdo))->access($userId,$tripId);$role=(string)($access['role']??'viewer');
    if(!in_array($role,['owner','co_planner','traveler'],true))throw new DomainException('Viewer role is read-only in Travel Day Copilot.');
