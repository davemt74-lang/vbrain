...
try{
    ...
    if($status==='dismissed'&&$execution->ready()){
        $current=$execution->executionForAction($userId,$tripId,$actionId);if($current&&in_array((string)($current['status']??''),['queued','agent_working','awaiting_approval','applied'],true))throw new DomainException('This Next Move is already in execution. Release queued work or reject the proposal instead of dismissing it.');
    }
