...
<section class="dashboard vb-autonomy"><div class="shell">
  ...
  <section class="vb-autonomy-summary">
    <article class="dashboard-card"><span>Autonomy</span><strong><?=empty($p['enabled'])?'Off':e($modeLabels[(string)$p['mode']]??'Observe only')?></strong><small><?=empty($p['enabled'])?'No trip-scoped #58 standing policy is active.':'Saved standing policy'?></small></article>
  ...
  <div class="vb-autonomy-layout">
    <form method="post" class="dashboard-card vb-autonomy-settings"><input type="hidden" name="_csrf" value="<?=e(csrf_token())?>"><input type="hidden" name="trip_id" value="<?=$tripId?>">
      ...
      <div class="vb-autonomy-modes"><?php foreach($modeLabels as $value=>$label):?><label><input type="radio" name="autonomy_mode" value="<?=$value?>" <?=($p['mode']??'observe')===$value?'checked':''?>><span><strong><?=e($label)?></strong><small><?php if($value==='observe'):?>This standing policy records state only; it does not start research or apply planning changes.<?php elseif($value==='research'):?>Vacation Brain may accept/start eligible Next Moves, but every proposed trip change still waits.<?php else:?>May also apply eligible low-risk planning additions that pass every limit below.<?php endif;?></small></span></label><?php endforeach;?></div>
      <p class="muted">This page controls the new trip-scoped #58 standing policy. The older Proactive Vacation Brain auto-research preference is separate and may still start research when enabled; manage it from Proactive Settings if you want that behavior off too.</p>
