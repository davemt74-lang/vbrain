...
if(strpos($service,"SELECT tc.user_id,tc.role,tc.rsvp,tc.status,u.display_name,u.username,u.avatar_url\n")===false){fwrite(STDERR,"Shared roster must use an explicit email-free projection.\n");exit(1);}
if(strpos($service,'u.avatar_url,u.email')!==false){fwrite(STDERR,"Shared roster must not fetch collaborator email addresses.\n");exit(1);}
...
if(strpos($service,"requireAccess(\$userId,\$tripId,'vote')")===false){fwrite(STDERR,"RSVP and vote participation must share the non-viewer capability gate.\n");exit(1);}
if(strpos($service,'ORDER BY id DESC LIMIT 1')!==false&&strpos($service,'DreamService($this->pdo))->addItem')!==false){fwrite(STDERR,"Shared item creation must use its own insert id rather than a latest-row lookup.\n");exit(1);}
...
$manage=$read('trip-collaboration.php');foreach(['Collaborative Trips & Travelers','Create invitation link','Copy this link now','Co-planner','Traveler — view, RSVP, vote','Viewer — read-only','revoke_invite','change_role','remove_member','Private Traveler Memory','can_vote'] as $needle){if(stripos($manage,$needle)===false){fwrite(STDERR,"Collaboration management UI missing {$needle}\n");exit(1);}}
$shared=$read('shared-trip.php');foreach(['Shared Trip Workspace','Shared workspace, narrow data boundary','Itinerary decisions','setVote','addItem','removeItem','Safe booking summary','confirmation codes','Booking & Action Execution controls','Viewer access is read-only','can_vote'] as $needle){if(stripos($shared,$needle)===false){fwrite(STDERR,"Shared trip workspace missing {$needle}\n");exit(1);}}
