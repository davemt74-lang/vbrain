...
foreach(['CREATE TABLE trip_agent_actions',"ENUM('open','accepted','dismissed','completed','superseded')",'uq_trip_agent_action_source','idx_trip_agent_action_queue','fk_trip_agent_action_user','fk_trip_agent_action_trip','fk_trip_agent_action_batch','actions_synced_at','actions_sync_error','idx_trip_agent_batch_actions_sync','UPDATE trip_agent_batches b',"j.agent_type='overview'","j.status IN ('queued','running')","'app_version','1.32'"] as $needle){if(strpos($migration,$needle)===false){fwrite(STDERR,"Action queue migration missing {$needle}\n");exit(1);}}

$service=$read('app/Services/TripAgentActionService.php');
foreach(['class TripAgentActionService','function syncCompletedOverviewBatches','function syncFromDashboard','function queueForTrip','function updateStatus','function counts','TripSupervisorService','dashboardForJob','actions_synced_at IS NULL','actions_sync_error',"j.agent_type='overview'","j.status='completed'",'batchSupervisor',"$batch['supervisor']",'sourceKey=$key',"status=IF(status='superseded','open',status)","status='open' AND source_key NOT IN","status IN ('open','accepted')","FIELD(status,'accepted','open')",'JSON_INVALID_UTF8_SUBSTITUTE','Trip Agent action sync failed for batch'] as $needle){if(strpos($service,$needle)===false){fwrite(STDERR,"Action queue service missing {$needle}\n");exit(1);}}
...
foreach(['require_auth','REQUEST_METHOD','verify_csrf()','TripAgentActionService','updateStatus','trip_id','action_id','return_tab','#next-moves','#agent-results','error_log','could not update that trip decision'] as $needle){if(strpos($endpoint,$needle)===false){fwrite(STDERR,"Trip action endpoint missing {$needle}\n");exit(1);}}

$partial=$read('partials/trip-action-queue.php');
foreach(['id="next-moves"','Decision queue','Next Moves','syncFromDashboard','csrf_token()','trip-action.php','I’m on it','Dismiss','Done','Release','Apply migration 040','temporarily unavailable','error_log'] as $needle){if(strpos($partial,$needle)===false){fwrite(STDERR,"Next Moves UI missing {$needle}\n");exit(1);}}
