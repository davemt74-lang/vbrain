...
foreach(['CREATE TABLE trip_agent_actions',"ENUM('open','accepted','dismissed','completed','superseded')",'uq_trip_agent_action_source','idx_trip_agent_action_queue','fk_trip_agent_action_user','fk_trip_agent_action_trip','fk_trip_agent_action_batch','actions_synced_at','idx_trip_agent_batch_actions_sync','UPDATE trip_agent_batches SET actions_synced_at=NOW()',"'app_version','1.32'"] as $needle){if(strpos($migration,$needle)===false){fwrite(STDERR,"Action queue migration missing {$needle}\n");exit(1);}}
