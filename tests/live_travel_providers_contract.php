...
foreach(['confirmation_code','private_notes',"['notes']",'["notes"]'] as $forbidden){if(strpos($provider,$forbidden)!==false){fwrite(STDERR,"Live provider bridge must not consume {$forbidden}.\n");exit(1);}}
...
$mainAgent=$read('app/Services/VacationAgentService.php');foreach(['LiveTravelAgentContextService','LIVE TRIP CONTEXT','saved provider snapshots','do not have a successful saved lodging snapshot','indicative airfare'] as $n){if(strpos($mainAgent,$n)===false){fwrite(STDERR,"Main Vacation Brain live-data grounding missing {$n}\n");exit(1);}}
