...
foreach(["addEventListener('click'",'navigator.geolocation','getCurrentPosition','anchor_mode','latitude','longitude','Coordinates will not be stored'] as $needle){if(strpos($js,$needle)===false){fwrite(STDERR,"Local Concierge one-time location JS missing {$needle}\n");exit(1);}}
if(preg_match('/(?:DOMContentLoaded|window\.onload|addEventListener\([\'\"]load)[\s\S]{0,800}getCurrentPosition/i',$js)){fwrite(STDERR,"Device geolocation must not run automatically on page load.\n");exit(1);}
