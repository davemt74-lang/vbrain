...
foreach(['CREATE TABLE trip_booking_action_intents','CREATE TABLE trip_booking_action_quotes','CREATE TABLE trip_booking_action_receipts','idempotency_key','provider_state_encrypted','provider_state_hash','current_quote_id','quote_digest','quote_expires_at','awaiting_approval','verification_pending',"'app_version','1.40'"] as $needle){if(strpos($m,$needle)===false){fwrite(STDERR,"Booking action migration missing {$needle}\n");exit(1);}}

$service=$read('app/Services/TripBookingActionService.php');
foreach(['class TripBookingActionService','ensureHandoffIntent','prepareHandoff','prepareBookingComCancellation','function approve','function execute','openHandoff','confirmHandoff','quoteDigest','quote_expires_at','provider_state_encrypted','aes-256-gcm','/orders/details/accommodations','/orders/cancel','Booking.com cancellation terms changed','Do not','verification_pending','updateBookingStatus','provider_handoff','booking_com_cancel','quote expired','locked quote','Payment-card data is never accepted or stored','provider-hosted checkout'] as $needle){if(stripos($service,$needle)===false){fwrite(STDERR,"Booking action service missing {$needle}\n");exit(1);}}
...
if(stripos($service,'rather than retry blindly')===false&&stripos($service,'rather than repeating')===false){fwrite(STDERR,"Direct destructive actions need an explicit no-blind-retry boundary.\n");exit(1);}
...
foreach(['TripBookingActionService','ensureHandoffIntent','transaction still requires its own locked quote and explicit approval'] as $needle){if(stripos($execution,$needle)===false){fwrite(STDERR,"Agent handoff integration missing {$needle}\n");exit(1);}}

$bootstrap=$read('app/bootstrap.php');if(strpos($bootstrap,"/Services/TripBookingActionService.php")===false){fwrite(STDERR,"TripBookingActionService is not loaded by bootstrap.\n");exit(1);}
