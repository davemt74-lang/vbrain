...
$m=$read('db/045_traveler_memory_graph.sql');foreach(['traveler_preference_controls',"ENUM('learn','ignore')",'correction_note',"'app_version','1.37'"] as $n){if(strpos($m,$n)===false){fwrite(STDERR,"Traveler Memory migration missing {$n}\n");exit(1);}}
$s=$read('app/Services/TravelerMemoryGraphService.php');foreach(['class TravelerMemoryGraphService','function snapshot','function dashboardSummary','function setSignalControl','function setTripLearning','function augmentTraits','function rerankDestinations','function nextTripSeed','function agentContext','function rawSignals','function controlMap','m.learning_enabled=1','m.status="complete"','Private trip notes are never included'] as $n){if(strpos($s,$n)===false){fwrite(STDERR,"Traveler Memory Graph service missing {$n}\n");exit(1);}}
if(strpos($s,'private_notes')!==false){fwrite(STDERR,"Traveler Memory Graph must not consume private trip notes.\n");exit(1);}
foreach(['user_id=? AND dream_trip_id=?','traveler_memory_correction','trip_memory_learning_changed'] as $n){if(strpos($s,$n)===false){fwrite(STDERR,"Traveler Memory ownership/audit missing {$n}\n");exit(1);}}
$page=$read('traveler-memory.php');foreach(['require_auth','verify_csrf()','Your Traveler Memory','Traveler DNA','Destination history','Spending intelligence','Travel history','Traveler DNA timeline','Ignore learned signal','Do this again, better','migration 045 / app v1.37'] as $n){if(strpos($page,$n)===false){fwrite(STDERR,"Traveler Memory page missing {$n}\n");exit(1);}}
