...
foreach(['final class TripAffordabilityService','snapshot','saveSettings','saveScenario','deleteScenario','inboxProjection','safeAgentContext','fitSuggestions','valueScore','saved_provider_estimate','learned_history','expected_final','expected_remaining','headroom','risk_status','confidence','does not guess exchange rates','max($b,$override)','Only the trip owner can change affordability assumptions or saved scenarios','planning estimates, not quotes',"'requires_action'=>true"] as $needle)if(strpos($s,$needle)===false){fwrite(STDERR,"Affordability service missing {$needle}\n");exit(1);}
