...
foreach(['final class TripAffordabilityService','snapshot','saveSettings','saveScenario','deleteScenario','inboxProjection','safeAgentContext','fitSuggestions','valueScore','saved_provider_estimate','learned_history','expected_final','expected_remaining','headroom','risk_status','confidence','does not guess exchange rates','max($b,$override)','Only the trip owner can change affordability assumptions or saved scenarios','Planning forecast only',"'requires_action'=>true",'storedSettings((int)$trip[\'user_id\'],$tripId)','$settings[\'include_learned_history\']=0','preg_match(\'/^\\d+$/\',$raw)'] as $needle)if(strpos($s,$needle)===false){fwrite(STDERR,"Affordability service missing {$needle}\n");exit(1);}
