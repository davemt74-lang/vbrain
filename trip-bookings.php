...
$userId=require_auth();$pdo=db();$tripId=(int)($_GET['id']??$_POST['trip_id']??0);$bookings=new TripBookingService($pdo);$bookingActions=new TripBookingActionService($pdo);$dreams=new DreamService($pdo);$error='';
...
$bookingActionRows=$bookingActions->ready()?$bookingActions->intentsForTrip($userId,$tripId,100):[];$bookingActionByBooking=[];foreach($bookingActionRows as $intent){$bid=(int)($intent['booking_id']??0);if($bid>0&&!isset($bookingActionByBooking[$bid]))$bookingActionByBooking[$bid]=$intent;}
$pageStyles=['assets/trip-bookings.css','assets/booking-actions.css'];$title='Booking & Readiness — '.$trip['name'];
...
function vb_booking_is_booking_com(array $booking): bool{$hay=strtolower((string)($booking['provider_name']??'').' '.(string)($booking['provider_url']??''));return str_contains($hay,'booking.com')||str_contains($hay,'booking_com');}
...
<div class="shell">
  ...
  <?php if($snapshot['ready']&&!$bookingActions->ready()):?><div class="trip-intel-alert"><strong>Controlled Booking & Action Execution is not enabled yet.</strong> <?php if(is_admin()):?><a href="<?=e(app_url('upgrade.php'))?>">Run System Upgrade for migration 048</a><?php else:?>An administrator needs to apply migration 048.<?php endif;?> Existing booking/readiness features remain available.</div><?php endif;?>
  ...
  <?php if($bookingActions->ready()):?><section class="dashboard-card vb-booking-section"><div class="vb-booking-section-head"><div><span class="eyebrow">Action execution</span><h2>Vacation Brain can prepare the next step — not silently buy it</h2><p>Provider checkout, reservations, purchases, modifications, and cancellations receive a separate locked quote and explicit transaction approval. Card numbers are never collected by Vacation Brain.</p></div><span class="vb-booking-safe"><?=count(array_filter($bookingActionRows,fn($x)=>in_array($x['status'],['awaiting_approval','approved','handoff_pending','verification_pending'],true)))?> need attention</span></div><?php if($bookingActionRows):?><div class="vb-booking-list"><?php foreach(array_slice($bookingActionRows,0,6) as $intent):?><article class="vb-booking-card"><div class="vb-booking-card-top"><div class="vb-booking-type"><?=e(vb_booking_label((string)$intent['action_type']))?> · <?=e(vb_booking_label((string)$intent['provider_slug']))?></div><span class="vb-booking-status"><?=e(vb_booking_label((string)$intent['status']))?></span></div><div class="vb-booking-card-main"><div><h3><?=e(vb_booking_label((string)$intent['adapter_mode']))?></h3><p><?=e((string)($intent['quote']['terms']?:'Prepare the current provider quote to continue.'))?></p></div></div><div class="vb-booking-actions"><a class="button secondary small" href="<?=e((string)$intent['review_url'])?>">Review action</a></div></article><?php endforeach;?></div><?php else:?><p>No provider actions have been prepared yet.</p><?php endif;?></section><?php endif;?>
  ...
  <section class="dashboard-card vb-booking-section">
    ...
    <div class="vb-booking-list">
      <?php foreach($bookingRows as $booking):$intent=$bookingActionByBooking[(int)$booking['id']]??null;?>
      <article class="vb-booking-card status-<?=e((string)$booking['status'])?>">
        ...
        <?php if($intent):?><div class="vb-live-confirmation"><strong>Provider action:</strong> <?=e(vb_booking_label((string)$intent['status']))?> · <a href="<?=e((string)$intent['review_url'])?>">review execution</a></div><?php endif;?>
        ...
        <div class="vb-booking-actions">
          ...
          <?php if($bookingActions->ready()&&in_array((string)$booking['status'],['unbooked','ready_to_book','changed'],true)):?><a class="button primary small" href="<?=e(app_url('booking-action.php?id='.$tripId.'&booking='.(int)$booking['id']))?>"><?= $intent?'Review booking action':'Prepare booking action' ?></a><?php endif;?>
          <?php if($bookingActions->ready()&&vb_booking_is_booking_com($booking)&&$booking['booking_type']==='lodging'&&in_array((string)$booking['status'],['booked','confirmed','changed'],true)):?><a class="button secondary small" href="<?=e(app_url('booking-action.php?id='.$tripId.'&booking='.(int)$booking['id'].'&mode=cancel'))?>">Check cancellation</a><?php endif;?>
