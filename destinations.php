...
if($hasGallery){
    ...
    $galleryImageSelect="(SELECT gi.image_url FROM destination_gallery_images gi WHERE gi.destination_catalog_id=d.id ORDER BY {$galleryOrder}gi.sort_order ASC,gi.id ASC LIMIT 1) AS gallery_image_url";
}else{
    $galleryImageSelect="NULL AS gallery_image_url";
}
$sql="SELECT d.*, $galleryImageSelect FROM destination_catalog d WHERE d.status='active'".$sampleClause;$args=[];
