...
<div class="sample-toggle-card dashboard-card"><div><h2>Sample data is <?=$enabled?'ON':'OFF'?></h2><p class="muted">Profiles, destination listings, demo destination reports, and merch catalog items <?=$enabled?'are currently visible':'are currently hidden'?>.</p></div><form method="post"><input type="hidden" name="_csrf" value="<?=e(csrf_token())?>"><label class="sample-switch"><input type="checkbox" name="enabled" value="1" <?=$enabled?'checked':''?>><span></span><b>Show sample data</b></label><button class="button primary small">Save setting</button></form></div>
<div class="admin-stats" style="margin-top:20px"><div class="info-card"><strong><?=$counts['users']?></strong><span>sample users</span></div><div class="info-card"><strong><?=$counts['profiles']?></strong><span>Travel Match profiles</span></div><div class="info-card"><strong><?=$counts['destinations']?></strong><span>destinations</span></div><div class="info-card"><strong><?=$counts['reports']?></strong><span>destination reports</span></div><div class="info-card"><strong><?=$counts['merch']?></strong><span>merch products</span></div><div class="info-card"><strong><?=$counts['places']?></strong><span>catalog places</span></div><div class="info-card"><strong><?=$counts['entities']?></strong><span>research entities</span></div></div>
<div class="dashboard-card" style="margin-top:20px"><h2>What this sample pack includes</h2><div class="admin-quick-grid"><div class="info-card"><h3>10 Travel Match people</h3><p>Complete 18+ opt-in profiles, generated profile photos, prompts, travel traits, Dream Trips, and questionnaire activity.</p></div><div class="info-card"><h3>10 visual destinations</h3><p>Santorini, Kyoto, Tulum, Aspen, Bali, Amalfi Coast, Maui, Paris, Sedona, and Costa Rica with generated listing imagery.</p></div><div class="info-card"><h3>Destination-report demos</h3><p>Each curated destination gets a sample report layout so the research UI is useful before the first live LLM web-research request.</p></div><div class="info-card"><h3>Expanded merch</h3><p>Tees, hoodies, cap, mug, tote, crewneck, stickers and tumbler entries for the ecommerce shell.</p></div></div><p class="microcopy">Sample accounts use non-public <code>@vacationbrain.local</code> addresses and an unknown random password hash. They are intended for discovery/demo data, not login.</p></div></div></section>
